...
int UnixSignalNotifierDispatcher::m_signalPipeWriteFd = -1;


void UnixSignalNotifierDispatcher::signalHandler(int signalNumber)
{
	if (m_signalPipeWriteFd >= 0)
		::write(m_signalPipeWriteFd, &signalNumber, sizeof(int));
}
	...
	, m_pipeNotifier(nullptr)
{
	...
	m_pipeReadFd = fds[0];
	m_signalPipeWriteFd = fds[1];

	//
	m_pipeNotifier = new QSocketNotifier(m_pipeReadFd, QSocketNotifier::Read, this);
...
UnixSignalNotifierDispatcher::~UnixSignalNotifierDispatcher()
{
	...
	int pipeWriteFd = m_signalPipeWriteFd;
	m_signalPipeWriteFd = -1;
...
void UnixSignalNotifierDispatcher::onPipeActivated(int fd)
{
	...
	// emit the activated signal with the number
...
// -----------------------------------------------------------------------------
/*!
	Constructs a unix socket notifier with the given \a parent.  It's
	activated() signal will be emitted when the signal \a unixSignal is raised.

	Unix socket notifiers are created with the notifier enabled, to disable
	...
	, m_enabled(false)
{
	...
	UnixSignalNotifierDispatcher *dispatcher = UnixSignalNotifierDispatcher::instance(true);
	if (!dispatcher) {
...
{
	if (m_enabled) {
		UnixSignalNotifierDispatcher *dispatcher = UnixSignalNotifierDispatcher::instance();
		if (dispatcher)
...
/*!
	...
	Slot called by the \a UnixSignalNotifierDispatcher when any enabled signal
