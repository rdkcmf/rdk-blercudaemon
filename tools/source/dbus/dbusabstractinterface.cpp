	...
	, m_propChangedConnected(false)
{
	...
	if (!conn.connect(QDBusAbstractInterface::service(),
...
/*!
	...
	The notify signal may either have no args or the a single arg that matches
                                                ...
                                                const QStringList& invalidatedProperties)
{
	...
	for (; it != changedProperties.end(); ++it) {
		...
		if (propIndex < 0) {
			qWarning() << "odd, no meta property for" << propName;
			continue;
		}
		...
		if (!propMeta.hasNotifySignal()) {
			qDebug() << "skipping property change notification for" << propName;
			continue;
		}

		const QMetaMethod notifyMethod = propMeta.notifySignal();
		if (Q_UNLIKELY(!notifyMethod.isValid())) {
			qWarning() << "odd, invalid property notify signal";
			continue;
		}

		// invoke the notify signal
		invokeNotifySignal(notifyMethod, propValue, propMeta.userType());
	}
                                               ...
                                               int propertyType)
{
	...
	if (Q_UNLIKELY(method.parameterCount() > 1)) {
		qWarning() << "to many parameters for property notify signal";
		return;
	}
	...
	if (Q_UNLIKELY(propertyType != method.parameterType(0))) {
		qWarning() << "notify signal takes inconsistant parameter type";
		return;
	}
	...
	// if the received property type is a bit more complex ...
		...
		if (dbusValue.currentSignature() != expectedSignature) {
			qWarning() << "mismatch signature on property notify signal, expected"
			           << expectedSignature << "actual" << dbusValue.currentSignature();
			...
			if (!QDBusMetaType::demarshall(dbusValue, propertyType, result.data()) ||
				!result.isValid()) {
				qWarning() << "failed to demarshall property value";
